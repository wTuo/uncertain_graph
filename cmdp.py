...
def compare_strategies(PIs):
    ...
    def strategy_equivalence(PIa,PIb):
        ...
        if apply_strategy(PIa[0],PIb[1],PIb[2]):
            return True
        else:
            return False
...
def vis_PI(PI, undetected, q, title):
    PI_g= pydot.Dot(graph_type='digraph',label=title)
    node_id=0
    st=[]
...
def mdp(G, s, d, nodes, existingedges, undetected):
    ...
    def getPI(G, s, d):
        ...
        q = [-99999999 for item in xrange(3**len(undetected))]
...
def apply_strategy(PI, q, undetected):
    ...
    theory_optimal = -q[3**len(undetected)-1]
    if str(theory_optimal) != str(mean) :
        return False
    else:
        print theory_optimal, mean
        return True
...
if __name__ == '__main__':
    ...
    for p in np.arange(0.01, 1.0, 0.01):
        graphinfo = gengraph(edgelist,0,7, p)
